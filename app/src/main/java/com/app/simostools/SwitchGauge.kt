...
class SwitchGauge : androidx.appcompat.widget.AppCompatTextView {
        ...
        if (showGraduations) {
            ...
            for (i in 1 until 10) {
                drawLine(
                    vertical,
                    i.toFloat() * 10f,
                    progressPaint.strokeWidth + margin * 1.5f,
                    canvas,
                    tickPaint
                )
            }
    ...
    private fun drawRound(canvas: Canvas) {
        ...
        if (centerGauge) {
            if (currentProgress < 50.0f) {
                drawCircle(
                    minAngle + angleProgress,
                    maxAngle * 0.5f - angleProgress,
                    canvas,
                    progressPaint
                )
            } else {
    ...
    private fun drawCircle(start: Float, finish: Float, canvas: Canvas, paint: Paint) {
        ...
        rect.set(
            margin_h + strokeWidth,
            strokeWidth,
            currentWidth - strokeWidth - margin_h,
            currentHeight - strokeWidth
        )
