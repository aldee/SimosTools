...
object FlashUtilities {
    ...
    fun splitBinBlocks(bin: ByteArray): Array<ByteArray> {
        ...
        for (i in 1..5) {
            splitBlocks[i] = bin.copyOfRange(
                boxCode!!.software.fullBinLocations[i],
                boxCode.software.fullBinLocations[i] + boxCode.software.blockLengths[i]
            )
        }
    ...
    fun checksumSimos18(bin: ByteArray, baseAddress: UInt, checksumLocation: Int): checksummedBin {
        DebugLog.d(
            TAG,
            "Checksumming block, base address $baseAddress, and checksum location: $checksumLocation"
        )
        ...
        var startAddress1 = byteArrayToInt(
            bin.copyOfRange(checksumLocation + 12, checksumLocation + 16).reversedArray()
        ).toUInt() - offset
        var endAddress1 = byteArrayToInt(
            bin.copyOfRange(checksumLocation + 16, checksumLocation + 20).reversedArray()
        ).toUInt() - offset
        var startAddress2 = byteArrayToInt(
            bin.copyOfRange(checksumLocation + 20, checksumLocation + 24).reversedArray()
        ).toUInt() - offset
        var endAddress2 = byteArrayToInt(
            bin.copyOfRange(checksumLocation + 24, checksumLocation + 28).reversedArray()
        ).toUInt() - offset
        ...
        var checksumCalculated = byteArrayOf(
            0x0.toByte(),
            0x0.toByte(),
            0x0.toByte(),
            0x0.toByte()
        ) + intToByteArray(crc).reversedArray()
    ...
    fun checksumECM3(bin: ByteArray, range: IntArray): checksummedBin {
        ...
        //Starting Value
        var checksum = byteArrayToInt(
            bin.copyOfRange(checksumLocation + 8, checksumLocation + 12).reversedArray()
        ).toULong() shl 32;
        checksum += byteArrayToInt(
            bin.copyOfRange(checksumLocation + 12, checksumLocation + 16).reversedArray()
        ).toUInt()
        ...
        var checksumCalculated =
            intToByteArray((checksum shr 32).toInt()).reversedArray() + intToByteArray((checksum.toInt() and 0xFFFFFFFF.toInt())).reversedArray()
    ...
    fun encodeLZSS_orig(
        ...
        while (i <= input.size - 1) {
            ...
            index =
                searchBuffer.findFirst(checkCharacters) //The index where the bytes appear in the search buffer
            ...
            if (index == -1 || i == input.size - 1 || checkCharacters.size > 63) {
                ...
                if (checkCharacters.size > 2 && i < input.size - 1) {
                    ...
                    index = searchBuffer.findFirst(
                        checkCharacters.copyOfRange(
                            0,
                            checkCharacters.size - 1
                        )
                    )
                    ...
                    var offset =
                        ((searchBuffer.size - length - index))//Calculate the relative offset
                    ...
                    if (offset == 0) {
                        ...
                        outputBuffer += byteArrayOf(
                            ((offset shr 8) or (length shl 2)).toByte(),
                            (offset and 0xFF.toInt()).toByte()
                        )
    ...
    fun encodeLZSS(
        ...
        while (inputIterator.hasNext()) {
            ...
            if (index == -1 || index > searchBuffer.size - 3 || checkCharacters.size > 63 || i >= input.size - 1) {
                index =
                    searchBuffer.findFirst(checkCharacters.copyOfRange(0, checkCharacters.size - 1))
                ...
                if (length == 0) {
                    ...
                    if (debug) println(
                        "    -> Adding byte to outputBuffer: " + "%02x".format(
                            checkCharacters[0]
                        )
                    )
    ...
    public class Sa2SeedKey(inputTape: ByteArray, seed: ByteArray) {
        ...
        fun add() {
            ...
            carry_flag = 0.toUInt()
            var operands =
                instructionTape.copyOfRange(instructionPointer + 1, instructionPointer + 5)
        ...
        fun for_loop() {
            ...
            var operands =
                instructionTape.copyOfRange(instructionPointer + 1, instructionPointer + 2)
    ...
    fun buildWorkshopCode(
        bin: Array<ByteArray>,
        binAswVersion: COMPATIBLE_BOXCODE_VERSIONS
    ): ByteArray {
