        ...
        mData?.let { dataList ->
            ...
            for (i in 0 until dataList.count()) {
                ...
                pidItem?.let {
                    if (pidItem.data.count() > 0 && pidItem.enabled) {
                        ...
                        val incrementX =
                            mCurrentWidth / pidItem.data.count() * (mZoomAmount + mZoomingAmount)
                        ...
                        for (x in 0 until pidItem.data.count()) {
                            canvas.drawLine(
                                x * incrementX + startX,
                                startY - lastY,
                                (x + 1) * incrementX + startX,
                                startY - (pidItem.data[x] * incrementY),
                                linePaint
                            )
            ...
            for (i in 0 until dataList.count()) {
                dataList[i]?.let { data ->
                    if (data.enabled) {
                        var xPosition =
                            ((mCursorPosition / mCurrentWidth) * data.data.count()).toInt()
